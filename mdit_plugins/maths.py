...
"""
Patched version of mdit_py_plugins.dollarmath.dollarmath_plugin as defined at
https://github.com/executablebooks/mdit-py-plugins/blob/master/mdit_py_plugins/dollarmath/index.py
"""
...
def dollarmath_plugin(
    ...
    *,
    ...
    renderer: Callable[[str, RendererDict], str] | None = None,
    ...
    _renderer: Callable[[str, RendererDict], str] = (
...
def math_block_dollar(
    ...
    def _math_block_dollar(
        state: StateBlock, start_line: int, endLine: int, silent: bool
    ) -> bool:
        ...
        if len(line_text.strip()) > 3:
            if line_text.strip().endswith("$$"):
                ...
                eqno_match = DOLLAR_EQNO_REV.match(line_text[::-1])
                if eqno_match:
                    have_end_marker = True
                    label = eqno_match.group(1)[::-1]
                    end = end - eqno_match.end()
        ...
        state.line = next_line + (1 if have_end_marker else 0)

        token = state.push("math_block_label" if label else "math_block", "math", 0)
        token.block = True
        token.content = "\n".join(
            line[state.blkIndent :]
            for line in state.src[start_pos + 2 : end].split("\n")
        )
        token.markup = "$$"
